...
telegram_messages_sent_total = Counter(
    ...
    ["type"],  # type: notification, response, error
)
...
try:
    ...
    bot_info.info({
        ...
        "redis_configured": "true" if os.getenv("REDIS_URL", "not_set") != "not_set" else "false",
        "openai_configured": "true" if os.getenv("OPENAI_API_KEY", "not_set") != "not_set" else "false",
    })
except Exception:
...
@contextmanager
def count_operation(counter: Counter, *labels: str, status: str = "success") -> Iterator[None]:
