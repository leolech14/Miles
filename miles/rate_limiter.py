...
import redis
from miles.metrics import redis_operations_total, redis_response_duration
...
# Default rate limits
DEFAULT_LIMITS = {
    RateLimitType.TELEGRAM_COMMAND: RateLimit(requests=10, window=60, burst=5),    # 10/min, burst 5
    RateLimitType.OPENAI_REQUEST: RateLimit(requests=20, window=60, burst=3),      # 20/min, burst 3
    RateLimitType.SOURCE_SCAN: RateLimit(requests=5, window=300, burst=2),         # 5/5min, burst 2
    RateLimitType.PLUGIN_EXECUTION: RateLimit(requests=30, window=60, burst=10),   # 30/min, burst 10
    RateLimitType.REDIS_OPERATION: RateLimit(requests=1000, window=60, burst=100), # 1000/min, burst 100
    RateLimitType.USER_OPERATION: RateLimit(requests=30, window=60, burst=10),     # 30/min per user, burst 10
}
...
class RateLimiter:
    ...
    async def _check_redis_limit(
        ...
        try:
            ...
            if current_count + cost <= limit.requests:
                ...
                if oldest_request:
                    retry_after = min(retry_after, limit.window - (current_time - oldest_request[0][1]))
    ...
    async def _check_local_limit(
        ...
        if len(bucket) + cost <= limit.requests:
            ...
            retry_after = limit.window
            if bucket:
                retry_after = min(retry_after, limit.window - (current_time - bucket[0]))
    ...
    async def get_stats(self, limit_type: RateLimitType, identifier: str = "global") -> Dict[str, Any]:
        """Get current rate limit statistics."""
        limit = self.limits.get(limit_type)
        if not limit:
            return {"error": "No limit configured"}
        
        allowed, metadata = await self.is_allowed(limit_type, identifier, cost=0)  # Cost 0 = just check
