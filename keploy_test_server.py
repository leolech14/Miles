...
from miles.promo_store import get_promo_store, PromoNotifier
...
async def manual_scan(request: ScanRequest) -> ScanResponse:
    ...
    try:
        # Mock the scan function for testing
        seen: set[str] = set()
        
        # Simulate some test alerts
        test_alerts = [
            (request.min_bonus + 10, "test.com", f"Test {request.min_bonus + 10}% bonus"),
            (request.min_bonus + 20, "example.com", f"Test {request.min_bonus + 20}% bonus")
        ]
