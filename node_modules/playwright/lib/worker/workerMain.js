...
class WorkerMain extends import_process.ProcessRunner {
  constructor(params) {
    ...
    // Suites that had their beforeAll hooks, but not after all hooks executed.
    // These suites still need after all hooks to be executed for the proper cleanup.
  ...
  async _runTest(test, retry, nextTest) {
    ...
    const hasAfterAllToRunBeforeNextTest = reversedSuites.some((suite) => {
      return this._activeSuites.has(suite) && !nextSuites.has(suite) && suite._hooks.some((hook) => hook.type === "after all");
    });
  ...
  async _runAfterAllHooksForSuite(suite, testInfo) {
    ...
    await this._runAllHooksForSuite(suite, testInfo, "after all");
  }
