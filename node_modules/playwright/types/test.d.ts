...
export interface TestType<TestArgs extends {}, WorkerArgs extends {}> {
   ...
   * cannot be used in a `beforeAll` or `after all` hook. Use
   ...
   *   ```
   *
   * - Changing timeout for a `beforeAll` or `after all` hook. Note this affects the hook's timeout, not the test
   *   timeout.
   *
   *   ```js
   ...
   *
   * You can use [test.after all([title, hookFunction])](https://playwright.dev/docs/api/class-test#test-after-all) to
  ...
  /**
   * Declares an `after all` hook that is executed once per worker after all tests.
   *
   ...
   * **Details**
   *
   * When multiple `after all` hooks are added, they will run in the order of their registration.
   *
   * Note that worker process is restarted on test failures, and `after all` hook runs again in the new worker. Learn
   * more about [workers and failures](https://playwright.dev/docs/test-retries).
   *
   * Playwright will continue running all applicable hooks even if some of them have failed.
   * - `test.after all(hookFunction)`
   * - `test.after all(title, hookFunction)`
   *
   * **Usage**
   *
   * ```js
   ...
   * [TestInfo](https://playwright.dev/docs/api/class-testinfo).
   */
  after all(inner: (args: TestArgs & WorkerArgs, testInfo: TestInfo) => Promise<any> | any): void;
  /**
   * Declares an `after all` hook that is executed once per worker after all tests.
   *
   * When called in the scope of a test file, runs after all tests in the file. When called inside a
   ...
   * [TestInfo](https://playwright.dev/docs/api/class-testinfo).
   */
  after all(title: string, inner: (args: TestArgs & WorkerArgs, testInfo: TestInfo) => Promise<any> | any): void;
  /**
 ...
 * [test.after all([title, hookFunction])](https://playwright.dev/docs/api/class-test#test-after-all) hooks, and
...
interface TestConfigWebServer {
  ...
  /**
   * If true, it will reuse an existing server on the `port` or `url` when available. If no server is running on that
