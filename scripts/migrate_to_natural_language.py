...
Helps users transition from command-based to natural language interface
and validates the new configuration.
"""
...
def test_openai_connection() -> bool:
    ...
    try:
        # Test if conversation manager can initialize
...
def test_basic_functions() -> bool:
    ...
    try:
        ...
        if result.get("success"):
            print(
                f"✅ Source listing working ({result.get('total_sources', 0)} sources)"
            )
...
def show_example_conversations():
    """Show example natural language conversations."""
    print(
        """
