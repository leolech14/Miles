...
**Purpose** – Build a modular, open-source system that watches Brazilian loyalty programs, detects transfer-bonus promos and cheap award fares, predicts upcoming bonus windows, and notifies users via Telegram (plus a minimal web UI). All heavy lifting can run in a single Docker container, scales horizontally if needed, and is easily extended via plug-in folders.
...
- `plugins/` implement one capability (scrape source, analyze, alert). They declare themselves through Python entry-points so they can be hot-swapped without editing core.
...
Copy `.env.example` → `.env` and fill in secrets; Docker Compose automatically loads them.
...
class Promo(dict):
    """Normalized record persisted to DB (see models.Promo)."""
...
`core.plugin_loader` will auto-discover, filter by `PLUGINS_ENABLED`, and schedule jobs.
...
| `/addroute GRU JFK …` | monitor award price (add route)             |
| `/setnotify 08 22`    | daily quiet hours (set notification window) |
| `/toggle_ai on`       | enable free-text ChatGPT interpreter        |
| `/forecast`           | probability table from pattern miner        |

Unmatched messages are routed to OpenAI with a function-calling schema that can safely modify the DB.
...
```
Collectors → Parser → Scorer → De-duplicator → Notifier
```
...
When a transfer bonus **and** a cheap award seat align, `core.notifier` crafts a combo message.
...
4. Register entry-point in `pyproject.toml` (project configuration file).
...
- LLM summarizer – weekly digest of missed promos & forecasts.

---

## 15 License & Credits

Apache 2.0. Inspired by Smiles Helper, Buscador Smiles (a Brazilian tool), and the mileage-hacker community.

“Open, modular, relentlessly automated.” Drop your plug-in folder, commit, and watch your Telegram light up 🚀
