...
def create_insights_report(analysis: Dict[str, Any]) -> str:
    ...
    for source_data in analysis['best_sources']:
        ...
        report.append(f"- **{source}**: {total} promos, {avg_bonus:.1f}% avg bonus (Score: {score:.0f})")
    ...
    if len(daily_data) >= 2:
        ...
        report.append(f"- Success Rate: {latest[2]:.1f}% {trend_emoji} ({success_trend:+.1f}%)")
        report.append(f"- Response Time: {latest[3]:.2f}s {time_emoji} ({time_trend:+.2f}s)")
    ...
    report.append(f"- **Optimize {slowest_cmd[0]}**: Currently {slowest_cmd[2]:.2f}s avg response time")
    
    # Find least successful command
    least_successful = min(analysis['command_efficiency'], key=lambda x: x[3])
    report.append(f"- **Improve {least_successful[0]}**: Only {least_successful[3]:.1f}% success rate")
    
    # Best source recommendation
    best_source = analysis['best_sources'][0]
    report.append(f"- **Focus on {best_source[0]}**: Best value source with {best_source[1]} promos")
...
def main():
    ...
    try:
        ...
        # Step 4: Summary
        print("\\n🎯 WORKFLOW COMPLETE!")
        print(f"📁 Files created in all_pdfs/:")
        print(f"  - {usage_file}")
        print(f"  - {promo_file}") 
        print(f"  - {report_file}")
